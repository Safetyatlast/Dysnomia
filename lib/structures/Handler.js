...
class Handler {
  ...
  checkPermissions(message, command) {
    if (command.permissions.length > 0) {
      const botMember = message.channel.guild.members.get(this.client.user.id);
    ...
    for (let i = 0; i < command.arguments.length; i++) {
      ...
      if (argument.repeatable) {
        ...
        for (let o = 0; o < argument.maxRepeats; o++) {
          try {
            args[argument.name].push(await Promise.resolve(typeReader.run(splitContent[splitContentIndex], message, command, this)));
            splitContentIndex++;
          } catch (err) {
        ...
        try {
          args[argument.name] = await Promise.resolve(typeReader.run(splitContent[splitContentIndex], message, command, this));
          splitContentIndex++;
        } catch (err) {
          if (!argument.optional) return { Argument, Error: err };
