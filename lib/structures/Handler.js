...
module.exports = class Handler {
   ...
   *
   * @param {Eris.Message} msg
   * @returns {Promise<{MissingPermissions: (Eris.Guild|boolean), Error: symbol}|{Precondition: Precondition, Error: symbol}|{Error: symbol}>}
   */
  async run(msg) {
        ...
        if (!result.Command.checkCooldown(msg.author.id)) {
          ...
          if (MissingPermissions.length === 0) {
            ...
            if (!Precondition || Precondition.precondition) {
                msg = await this.executeMiddleware(msg, result.Command);
